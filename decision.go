...
func distanceTo(head Coordinates, target Coordinates) int32 {
	...
	*/
	fmt.Println("Distance between ", head, " and ", target)
	distance := int32(math.Sqrt((math.Pow(float64(target.X-head.X), 2) + (math.Pow(float64(target.Y-head.Y), 2)))))
...
func isHealthy(me BattleSnake, food []Coordinates) bool {
	...
	*/
	...
	return me.Health > (distanceTo(me.Head, closest))
}
